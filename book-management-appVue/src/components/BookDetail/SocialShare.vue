<template>
  <div class="social-share">
    ...
    <div class="share-buttons">
      <button @click="shareOnX" class="share-btn x">
...
const shareText = `${props.title} kitabını keşfedin!`

const shareOnX = () => {
  window.open(
    `https://twitter.com/intent/tweet?text=${encodeURIComponent(shareText)}&url=${encodeURIComponent(props.url)}`,
  )
}
...
const shareOnWhatsapp = () => {
  window.open(
    `https://api.whatsapp.com/send?text=${encodeURIComponent(shareText + ' ' + props.url)}`,
  )
}
</script>
